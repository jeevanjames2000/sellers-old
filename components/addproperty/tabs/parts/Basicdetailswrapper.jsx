...
import {
  ...
  puneLocalities,
  vizagLocalities,
} from "@/components/shared/customCities";
    ...
    const fetchLocalities = async () => {
      try {
        const response = await fetch(
          `https://api.meetowner.in/api/v1/search?query=${location}&city=Hyderabad`
        );
  ...
  return (
    <>
      <div className="py-2 bg-[#E2EAED]">
        <p className="text-md md:text-lg font-bold text-[#1D3A76] text-center">
          ADD BASIC DETAILS
        </p>
      </div>
      <div className="relative p-5 sm:p-10 h-[calc(100vh-260px)] sm:h-[calc(100vh-220px)] overflow-y-auto">
        <>
          <div className="flex gap-1 mb-4">
            <p className="text-[#1D3A76] text-[13px] font-sans font-medium">
              Property Type
            </p>
            ...
            {propertyInList.length > 0 &&
              propertyInList.map((property, index) => {
                return (
                  <div
                    key={index}
                    onClick={() => updatePropertyType(property.value)}
                    className={`group cursor-pointer px-4 py-1 sm:px-8 sm:py-2 rounded-md  ${
                    ...
                    <p
                      className={`${
                        propertyType === property.value
                          ...
                          : "text-[#1D3A76] text-[10px] font-semibold group-hover:text-white"
                      }`}
                    >
                      {property.name}
                    </p>
          ...
          {propertyTypeError && (
            <p className="text-red-500 text-[10px] mt-2">{propertyTypeError}</p>
          )}
        </>
        <>
            ...
            {propertyForList.length > 0 &&
              propertyForList.map((property, index) => {
                return (
                  <div
                    key={index}
                    onClick={() => updateLookingTo(property.value)}
                    className={`group cursor-pointer px-4 py-1 sm:px-8 sm:py-2 rounded-md  ${
                      lookingTo === property.value
                        ? "border border-[#1D3A76] bg-[#1D3A76]"
                        : "border border-[#909090]  hover:bg-[#1D3A76]"
                    }`}
                  >
          ...
          {lookingToError && (
            <p className="text-red-500 text-[10px] mt-2">{lookingToError}</p>
          )}
          {lookingTo === 1 && (
            <div className="my-5 w-full sm:w-[40%]">
              <Select
                label="Transaction Type"
                labelClassName="!text-[#1D3A76] !text-[13px] font-sans font-medium"
        ...
        {locationError && (
          <p className="text-red-500 text-[10px] mt-2">{locationError}</p>
        )}
        {showDropdown && localites.length > 0 && (
          <ul className="w-full bg-white border border-[#1D3A76] rounded-md shadow-lg max-h-48 overflow-auto z-50">
            {localites.map((locality, index) => (
              <li
                key={index}
                onClick={() => handleLocationSelect(locality.locality)}
      ...
      <div className="flex flex-row justify-end items-center mb-3">
        <div
          ...
          className="border border-[#1D3A76] bg-[#1D3A76] px-8 py-2 mr-2 rounded-md cursor-pointer"
        >
